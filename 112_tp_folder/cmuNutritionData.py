#gives cals, carbs, protein, fat for CMU foods
#data source: https://app.mynutritioncalculator.net/cmu/
import copy
import random
import math
...
# Updated Animation Starter Code, original code from cs.cmu.edu/~112/
...
def getNutInfo(dct):
    #turn csv string into Nutrition Info string
    result = ""
    nutrients = dct.split(',')
...
class Icon(object):
    ...
    def draw(self, canvas):
        ...
        canvas.create_rectangle(self.x, self.y, self.x+self.size, 
                                self.y+self.size, fill='turquoise3', width=5)
...
class Return(object):
    ...
    def draw(self, canvas):
        canvas.create_rectangle(0, 0, self.size, self.size, 
                                        fill='turquoise3', width=2)
